...
<<<<<<< ours
=======
- `fenologie/` — Fenologie Nieuwkoop: tien jaar Sentinel-2 NDVI per H3-hexagon (res 10, 4.353 cellen van 152 m) over de Nieuwkoopse Plassen & De Haeck. Klik een cel → reeks tegen de eigen DOY-klimatologie, z-anomalieën, decompositie en Theil-Sen/Mann-Kendall-trend. MapLibre, geen framework, eigen SVG-grafieken (geen Chart.js). Layout: twee zwevende panelen over een schermvullende kaart — links een inklapbaar bedieningspaneel (312 px, met legenda en herkomst), rechts het detailpaneel (460 px) dat pas bij een klik verschijnt; onder 820 px wordt dat laatste een bottom sheet. Een variant met een vaste detailkolom en een balk bovenlangs is geprobeerd en weer teruggedraaid, de zwevende opzet was de voorkeur. `fitToCube()` reserveert daarom 340 px links op brede schermen. Ondergrond is **PDOK**, geen CARTO: BRT Achtergrondkaart grijs (default) en standaard via `service.pdok.nl/kadaster/brt-achtergrondkaart/wmts/v2_0/<laag>/EPSG:3857/{z}/{x}/{y}.png`, plus de luchtfoto uit `hwh/luchtfotorgb`. De oude `a.basemaps.cartocdn.com`-URL's uit `shared/deckgl-utils.js` gaven een API-key-fout; de nieuwere viewers gebruiken `basemaps.cartocdn.com` zonder subdomein. Assets worden **absoluut** geladen (`/fenologie/js/app.js`), net als in `vraag-de-kaart` en `gebiedsviewer`: met `cleanUrls` staat er `/fenologie` zonder slash in de adresbalk, dus relatieve paden resolven naar de root en geven 404. Kaartuitsnede komt uit `fitToCube()` op de ring van de kubus, niet uit een vaste zoom. **Twee databronnen achter één klik**: binnen het gebied de voorberekende kubus onder `data/fenologie/`, **gesplitst in een index en shards**: `index.json` (1,4 MB / 319 kB gzipped) bevat alleen wat de kaart nodig heeft en laadt direct; `s/<h3-ouder>.json` (108 stuks, gemiddeld 92 kB) bevat de base64-Int16-reeksen en komt pas bij een klik binnen, gegroepeerd op de H3-ouder twee resoluties grover (`meta.shard_res`). `Cube.ensureSeries(cell)` is daarom asynchroon en `showCell` tekent de drie reeksgrafieken pas na die fetch, met een `seriesToken` zodat een snelle tweede klik de eerste niet overschrijft. `lat`/`lon` en `years` staan niet in de index, die leidt de browser af uit de H3-id en `meta.years`. Geen DuckDB nodig op deze schaal; daarbuiten `api/ndvi-series.js`, dat live openEO op Copernicus Data Space aanroept. Zonder `CDSE_CLIENT_ID`/`CDSE_CLIENT_SECRET` geeft dat endpoint 503 op `?probe=1` en verbergt de viewer de live-knop — de kubus blijft werken. Kubus bouwen met `scripts/build_fenologie_cube.py` (`--from-grass` binnen een GRASS-sessie op de STRDS'en van de HAS/PZH-pipeline, of `--demo` voor gesimuleerde reeksen met dezelfde verwerking; de viewer zet dan zelf een `demo-data`-badge). **Bemonstering**: `--sampling zonal` (default) rasteriseert de hexagonen tot zones (`v.import` + `v.to.rast`) en draait `t.rast.univar -e zones=` over de STRDS, wat per datum per cel een mediaan plus pixeltelling geeft; `npix` komt in de index en als tegel in het paneel. `--sampling centroid` bemonstert alleen het middelpunt (op res 10 is dat 1 van ruim 150 pixels) en de viewer zet er dan een waarschuwing bij. `--verify N` trekt N willekeurige cellen na met een losse `t.rast.what`: bij centroid horen de verschillen nul te zijn, bij zonal zeggen ze hoe heterogeen een cel is. `--habitat <vector> --habitat-field <kolom>` vult `hab` met het type dat het meeste oppervlak beslaat (`r.stats -c` over zone en habitatraster). **PDOK heeft geen bruikbare habitatkaart**: de Natura 2000-service van RVO is alleen begrenzing, en "Habitatrichtlijn verspreiding van habitattypen" is het EU-rapportageraster van 10x10 km; de echte habitattypenkaart zit in de NDVH bij BIJ12. Publiek alternatief: de beheertypen uit het provinciale Natuurbeheerplan, in dit repo al bereikbaar via `Landelijk_gebied_NBP_<jaar>` in `gebiedsviewer/config.js`. **Het hele GRASS-pad is nooit tegen een echte sessie gedraaid** - eerst `--verify` draaien. Methodiek uit *Monitoring habitatveranderingen Nieuwkoop* (HAS green academy + PZH, 2026): 365-daagse no-leap kalender, DOY-stappen van 5 d met venster ±10 d, spreiding 1,4826 × MAD, z-drempel SD > 0,02, trendtoets op de jaarmedianen (n = 10, níét op de 5-daagse reeks — die is zo autogecorreleerd dat Mann-Kendall p-waarden van 1e-19 geeft). De decompositie draait client-side en is bewust simpel (seizoen = referentiecurve, trend = lopende mediaan); een echte STL hoort in de GRASS-addon `t.rast.pointseries`, waar de volledige reeks staat.
>>>>>>> theirs
